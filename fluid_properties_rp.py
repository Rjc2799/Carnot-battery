...
def myREFPROPdll( peroperties_in , value_in_1 , value_in_2 , properties_out ,   fluid_name, fluid_comp , nameUnits = "MASS BASE SI", iMass=0 , iFlag=0  ):
    '''
    Calls refprop function REFPROPdll in a convenient way.
    Parameters
    ----------
    peroperties_in : string
        Possible values:
            Dliq, Dvap: densities of vapor/liquid in 2-phase regime
    value_in_1 : decimal
        DESCRIPTION.
    value_in_2 : TYPE
        DESCRIPTION.
    ...
    fluid_comp : TYPE
        DESCRIPTION.
    nameUnits : TYPE, optional
        DESCRIPTION. The default is "MASS BASE SI".
    iMass : TYPE, optional
    ...
    -------
    TYPE
        1st elemtent of return array.

    '''
    
    iUnits = RP.GETENUMdll(0, nameUnits).iEnum
    
    o = RP.REFPROPdll(fluid_name , peroperties_in, properties_out , iUnits,iMass,iFlag, value_in_1 , value_in_2, fluid_comp) 
...
a = myREFPROPdll("TP" , 273.15 , 2e5 , "Dliq", "Propane * Butane * Pentane", [0.7,0.1,0.2])
...
prop_pq_vec = np.vectorize(prop_pq) # does not work?
prop_Tq_vec = np.vectorize(prop_Tq) # does not work?