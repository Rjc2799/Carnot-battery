...
def charging_phase():
    ...
    print(f"m_glycol: {m_glycol:.3f}kg")
...
def plot_preheating_tankC_mixture(
    ...
    plt.text(h_dot[-1]-1.5, T_range[-1] - 7, f'Tank Hot2,chosen\n{Temp_D_test:.2f} K', color='red', fontsize=12, ha='left')
    ...
    plt.text(h_dot_B, T_final - 9, f'Tank Hot1,final\n{T_final:.2f} K', color='magenta', fontsize=15, ha='left')
...
def standstill_after_charging(chg):
    ...
    print(f"total_surface_area_A_B: {total_surface_area_A_B:.3f}sq m")
    ...
    print(f"total_surface_area_C_D: {total_surface_area_C_D:.3f}sq m")
    ...
    print(f"total_surface_area_E_F: {total_surface_area_E_F:.3f}sq m")
    ...
    U_overall = 1 / R_total
    print(f"U_overall: {U_overall:.3f}")
    ...
    heat_loss_in_B = chg["m_dot_water_2"] * cp_water * (Temp_at_t_tankB - T_i_tank_B)
    heat_loss_in_D = chg["m_dot_water_1"] * cp_water * (Temp_at_t_tankD - T_i_tank_D)
    heat_gain_in_F = chg["m_dot_glycol"] * cp_glycol_water * (Temp_at_t_tankF - T_i_tank_F)
...
def discharging_phase(chg, stand):
    ...
    orc_state1 = flp.prop_pq(p_orc_low, 0, RP=RP, composition=composition)
    ...
    orc_state3 = flp.prop_pq(p_orc_high,0 , RP=RP, composition=composition)
    orc_state4 = flp.prop_pq(p_orc_high,1 , RP=RP, composition=composition)
    ...
    orc_state7 = flp.prop_pq(p_orc_low, 1, RP=RP, composition=composition)
    orc_state8 = flp.prop_pq(p_orc_low, 0, RP=RP, composition=composition)
    ...
    Q_total_JI = Q_dot_orc_1_evap * discharging_time
    Q_total_HG = Q_dot_orc_2_evap * discharging_time
    ...
    print(f"m_dot_water_discharge_JI: {m_dot_water_discharge_JI:.3f}kg/s")
    print(f"m_dot_glycol_discharge_KL: {m_dot_glycol_discharge_KL:.3f}kg/s")
...
#     # =========================
    ...
    print(f"Turbine work: {W_turbine_total/1e6:.3f}MJ")
...
def plot_carnot_battery_cycle(
    ...
    plt.text(hdot_cond_2[0] + -0.08, T_cond_2[0], 'Mid2', ha='right', va='bottom', fontsize=9, color='black')
    plt.plot(hdot_hot[-1], T_hot[-1], 'o', color='red')
    plt.text(hdot_cond_2[-1], T_cond_2[-1], 'Mid1', ha='right', va='bottom', fontsize=9, color='black')
    plt.text(hdot_cond_1[0] + 0.2, T_cond_1[0], 'Hot2', ha='right', va='bottom', fontsize=9, color='black')
    ...
    plt.text(hdot_evap[0] + -0.035, T_evap[0] + -2.1, 'Cold2', ha='right', va='bottom', fontsize=9, color='black')
    plt.plot(hdot_evap[-1], T_evap[-1], 'o', color='darkblue')
    plt.text(hdot_evap[-1] + 0.3, T_evap[-1] + -1.5, 'Cold1', ha='right', va='bottom', fontsize=9, color='black')
    ...
    plt.text(hdot_evap_orc[0]-0.05, T_evap_orc[0]-1.6, 'Mid1', ha='right', va='bottom', fontsize=9, color='black')
    plt.plot(hdot_evap_orc[-1], T_evap_orc[-1], 'o', color='red')
    plt.text(hdot_evap_orc[-1], T_evap_orc[-1], 'Mid2', ha='right', va='bottom', fontsize=9, color='black')
    ...
    plt.text(hdot_evap_orc_2[0] + -0.08, T_evap_orc_2[0]-2.8, 'Hot1', ha='right', va='bottom', fontsize=9, color='black')
    plt.plot(hdot_evap_orc_2[-1], T_evap_orc_2[-1], 'o', color='red')
    plt.text(hdot_evap_orc_2[-1]+0.02, T_evap_orc_2[-1]-0.8, 'Hot2', ha='left', va='bottom', fontsize=9, color='black')
    ...
    plt.text(hdot_cond_orc[0] -0.1, T_cond_orc[0]+3.0, 'Cold1', ha='left', va='bottom', fontsize=9, color='black')
    plt.plot(hdot_cond_orc[-1], T_cond_orc[-1], 'o', color='darkblue')
    plt.text(hdot_cond_orc[-1], T_cond_orc[-1]+0.06, 'Cold2', ha='left', va='bottom', fontsize=9, color='black')
    ...
    plt.plot(entropy_control_hdot_extended, entropy_control_T_extended, 'k:', 
             linewidth=3, alpha=0.8, label='Rejected Excess Entropy')
...
{'='*80}
{'Estimated heat loss rate during storage in tank B:'.ljust(50)} {args['heat_loss_in_B']/1000:.4f} kJ/s
{'Estimated heat loss rate during storage in tank D:'.ljust(50)} {args['heat_loss_in_D']/1000:.4f} kJ/s
{'Estimated heat gain rate during storage in tank F:'.ljust(50)} {args['heat_gain_in_F']/1000:.4f} kJ/s
