...
def preheating(m_water_1, cp_water, Temp_B, T_surrounding, m_dot_wf, comp_out, throt_in, preheating_time=14400):
    ...
    Mc = r / (r + 1)      # Fraction of m_water_initial in tank C
    Md = 1 / (r + 1)      # Fraction of m_water_initial in tank D
...
def plot_preheating_tankC_mixture(
    ...
    h_dot = m_dot_water_initial * (np.array(h_range) - h_ref) / 1000  # kW
    ...
    h_dot_B = m_dot_water_initial * (
        flp.tp(T_final, p_wf_high, RP=RP, composition=composition)[2] - h_ref
    ) / 1000
    plt.plot(h_dot_B, T_final, 'mo', label=f'Target (after mixing): {T_final:.2f} K')
    plt.text(h_dot_B, T_final - 9, f'Target\n{T_final:.2f} K', color='magenta', fontsize=12, ha='left')
...
def plot_carnot_battery_cycle(
    ...
    T_evap_orc_2 = np.linspace(Temp_i , Temp_j, 50) 
